...
%We then sign  $W_{S}$ and $W_{D}$ by the difference in mean posterior $R_{0}$ under each distribution posterior mean under complete data. If either data source biases posterior $R_{0}$ upwards or downwards, $W_{\bullet}$ is then positive or negative accordingly. Signing $W_{\bullet}$ allows us to capture both the difference in posteriors and the direction of bias, resulting in a metric that is maximally descriptive. 
...
%Finally, we can quantify the disagreement in signal between each data source. We define disagreement with respect to the full data posterior, $D_{S,D|F}$ as the magnitude of the vector $(\overrightarrow{W_{S}, W_{D}})$ leading to each point in the plane. Values near zero indicate that the posteriors under data, sequence, and complete data are all near identical and classification of date or sequence driven is less meaningful. Larger values signify that one or both data sources drive differing posteriors and classification is more meaningful. We refer to this as dissonance as it scales with the 'disagreement' between posterior data sources. Representing points $(W_{S}, W_{D})$ in a plane, such as fig \ref{fig:fig1}B therefore displays dissonance and classification. We also consider $| W_{S} - W_{N} |$ as a quantification of disagreement between data and sequence posteriors without respect to the posteriors under full data in fig \ref{fig:figS1}.
