...
%---------------------------------------------------------------------------------------------------------
\section*{Discussion}
\textcolor{blue}{What i want to say: The key messages from the results is that the number of site patterns is key driving whether or not an analysis is date driven or seq driven. Note that we refer to evolutionary rate earlier, but really we are referring to the number of mutations $rGT$ where r is the rate, G is the genome size, and T is notionally the average evolutionary time between samples such that more mutations between tips can be aided by a larger genome despite a lower rate, or more diffuse sampling to resuce the liielihod of sampling identical sequences.} 

\textcolor{blue}{The reason why sequence data is in a sense the limiting-reagent of phylodymamic inference is that it must explore a larger state space of trees. Date data need to integrate of the state space of topologies given a sampling chronology, while sequence data probabilistically influence topology while inferring absolte and relative timong of tips - a far more expensive state space. While massive sampling efforts are laudable and useful for methods in genomic epidemiology outside of phylodynamics, we stress the importance of effective down-sampling from these massive databases must be equally scrutinised. For instance, it would be of minimal returning value to include samples that are both identical in sampling time and sequence, such as from a super spreading event.}
...
% what about the phylodynamic threshold in all of this
...
The key factor driving this pattern is that 
