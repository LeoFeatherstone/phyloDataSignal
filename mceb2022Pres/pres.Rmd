...
## Talk breakdown
> 0. [Preprint: Assessing the effects of date and sequence data in phylodynamics](https://www.biorxiv.org/content/10.1101/2022.06.07.495205v1)
> 1.    Quantify data and sequence effects under birth-death model
> 2.    Validation
> 3.    Outlook
...
## Project origins (Covid in Australia)
...
## Separating date and sequence information?
```{r include=FALSE}
...
## Output
![This is a figure.](../figures/postEg.pdf)
...
```
![This is a figure.](figs/compPosterior.pdf)
...
fig <- ggplot(df, aes(x, colour = g)) +
        ...
        annotate("label", x = 0.5, y = 0.75, label='W[D]==frac(1,10)*sum(d[i],"","")', parse=T, size=2.5, fill=alpha(viridis(4)[4], 0.7)) +
        xlab(expression(R[0])) + ylab('CDF') +
        theme_minimal() +
        theme(legend.position = 'none')

fig + annotation_custom(ggplotGrob(hist), xmin = 0, 1, ymin = 0.75, ymax = 0.9)
...
```{r include=F}
# attempt to rebuild with slider
library(plotly)
load('../combinedData.Rdata')
...
for(step in 1:9){
  aval[[step]] <-list(visible = FALSE,
                      name = paste0('<=', x[step], 'sites'),
                      x=dat[which(dat$sitePatterns <= x[step]),]$dateDataW,
                      y=dat[which(dat$sitePatterns <= x[step]),]$seqDataW,
                      z=as.numeric(dat[which(dat$sitePatterns <= x[step]),]$sitePatterns))

}
aval[9][[1]]$visible = TRUE

# create steps and plot all traces
steps <- list()
fig <- plot_ly()
for (i in 1:9) {
  fig <- fig %>% add_trace(x=aval[i][[1]]$x, y=aval[i][[1]]$y, name = aval[i][[1]]$name, showlegend = FALSE, hoverinfo = 'name', type='scatter', mode='marker', marker=list(color=aval[i][[1]]$z, cauto=0, cmin = 7, cmax=869, colorscale='Viridis', showscale=T, name='Site Patterns'))

  step <- list(args = list('visible', rep(FALSE, length(aval))),
               method = 'restyle', label=paste0('<=', x[i]))
  step$args[[2]][i] = TRUE  
  steps[[i]] = step 
}  
...
fig <- fig %>%
  layout(sliders = list(list(active = 8,
                             currentvalue = list(prefix = "Site Patterns <= "),
                             steps = steps)))
